...
internal static class KeyMapper
{
  ...
  static KeyMapper()
  {
    foreach (KeyValuePair<string, KeyCode> pair in ImGuiToUnity)
    {
      if (!UnityToImGui.ContainsKey(pair.Value))
        UnityToImGui.Add(pair.Value, pair.Key);
    }
  }
  ...
  /// </summary>
  /// <param name="imguiKey"></param>
  public static KeyCode ConvertImGuiToUnity(ImGuiKey imguiKey) => (ImGuiToUnity.TryGetValue(imguiKey.ToString(), out KeyCode unityKey)) ? unityKey : KeyCode.None;
